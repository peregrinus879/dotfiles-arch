...
- `render-markdown.nvim` is added beyond the `omarchy-nvim` baseline as a standalone plugin, using its default upstream setup for headless-safe Markdown rendering inside Neovim.
...
The following do **not** belong in `dotfiles-arch`:

- Windows Terminal configuration (belong in `dotfiles-wsl`)
- WSL clipboard integration using `clip.exe` and `powershell.exe` (belong in `dotfiles-wsl`)
- WSL bootstrap steps such as `/etc/wsl.conf` (belong in `dotfiles-wsl`)
- Any other Windows interoperability behavior (belong in `dotfiles-wsl`)
